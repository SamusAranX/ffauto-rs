...
pub(crate) fn ffmpeg_info(args: &InfoArgs) -> Result<()> {
    ...
    for stream in &probe.streams {
        ...
        if let Some(language) = language { extra_info.add(language) }
        if let Some(title) = title { extra_info.add(format!("\"{title}\"")) }
        if default { extra_info.add("default") }
        ...
        match codec_type {
            StreamType::Video => {
                ...
                if let Some(field_order) = &stream.field_order { format_info.add(field_order) }
                if let Some(color_range) = &stream.color_range { format_info.add(color_range) }

                let mut color_info: Vec<String> = Vec::new();
                if let Some(color_space) = &stream.color_space { color_info.add(color_space) }
                if let Some(color_primaries) = &stream.color_primaries { color_info.add(color_primaries) }
                if let Some(color_transfer) = &stream.color_transfer { color_info.add(color_transfer) }
